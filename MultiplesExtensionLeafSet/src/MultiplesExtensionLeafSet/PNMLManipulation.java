...
public class PNMLManipulation {
	...
	public void place(String name,int x,int y,CSS2Color color, boolean jeton) {
		if(jeton) {
			place(name,x,y,color,1);
		}
		else {
			place(name,x,y,color,0);
		}
	}
	public void place(String name,int x,int y,CSS2Color color, long jeton) {
		Long jet=new Long(jeton);
		try {
			place = new PlaceHLAPI(name,page);
			if(jeton>0) {
				place.setInitialMarkingHLAPI(new PTMarkingHLAPI(jet));
			}	
