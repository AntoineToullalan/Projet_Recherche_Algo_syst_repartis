...
public class AutomateNode {
	...
	private TransitionHLAPI firstinsert,moreinsert,associate,asksharing,isinsert,pannesent,panne;
	...
	public void communicNodes(){
		...
		for (int j=0; j<size; j++ ) {
			...
			if (j!=num) {
				...
				manip.transition(name+"SentFailure"+j, x+3*tailleauto/4+300+(j)*ecart, b+8*ecart+(j+1)*100,CSS2Color.GREEN);
				pannesent = manip.getTransition();
				manip.arc(true,manip.getPlace(),manip.getTransition());
				manip.arc(false,nOshare,manip.getTransition());
				...
				manip.transition(name+"ReceiveFailure"+j, x-2*ecart+tailleauto/4+(j)*ecart, b+11*ecart+(j+1)*150-ecart,CSS2Color.GREEN);
				panne = manip.getTransition();
				manip.arc(false,nOshare,manip.getTransition());
				manip.arc(true,manip.getPlace(),manip.getTransition());
	...
	public TransitionHLAPI getpanne(){
		return panne;
	}

	public TransitionHLAPI getpannesent(){
		return pannesent;
	}
